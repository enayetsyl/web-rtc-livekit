...
app.post('/api/mod/mute', async (req, res) => {
  try {
    ...
    if (!roomName || !identity || !kind) return res.status(400).json({ error: 'roomName, identity, kind required' });
    ensureRole(role); ensureAdminish(role);
    ...
    res.json({ ok: true, tracksAffected: selected.map(t => ({ sid: t.sid, type: t.type, source: t.source })) });
  } catch (e) {
    res.status(e?.status || 500).json({ error: e?.message || 'Failed to mute' });
  }
});
...
app.post('/api/mod/screenshare', async (req, res) => {
  try {
    ...
    if (!roomName || !identity || allow === undefined) return res.status(400).json({ error: 'roomName, identity, allow required' });
    ensureRole(role); ensureAdminish(role);
    ...
    const ensure = (s) => { if (!nextSources.includes(s)) nextSources.push(s); };
    const remove = (s) => { nextSources = nextSources.filter(x => x !== s); };
    ...
    res.status(e?.status || 500).json({ error: e?.message || 'Failed to update screen-share permission' });
  }
});
...
app.post('/api/mod/screenshare/request', async (req, res) => {
  ...
  if (!pendingShareRequests.has(roomName)) pendingShareRequests.set(roomName, new Map());

  pendingShareRequests.get(roomName).set(identity, { name: name || identity, at: Date.now() });
...
app.get('/api/mod/screenshare/pending', async (req, res) => {
  ...
  const items = Array.from(map.entries()).map(([identity, v]) => ({ identity, ...v }));
...
app.post('/api/mod/screenshare/decide', async (req, res) => {
  try {
    ...
    if (!roomName || !identity || allow === undefined) {
      return res.status(400).json({ error: 'roomName, identity, allow required' });
    }
    ensureRole(role); ensureAdminish(role);
    ...
    const add = s => { if (!nextSources.includes(s)) nextSources.push(s); };
    const rm  = s => { nextSources = nextSources.filter(x => x !== s); };
    ...
    if (!shareDecisionsByRoom.has(roomName)) shareDecisionsByRoom.set(roomName, new Map());
  shareDecisionsByRoom.get(roomName).set(identity, allow ? 'allowed' : 'denied');

    res.json({ ok: true, allow, canPublishSources: nextSources });
  } catch (e) {
    res.status(e?.status || 500).json({ error: e?.message || 'Failed to decide screen-share' });
  }
});
...
app.get('/api/participant/canshare', async (req, res) => {
  try {
    ...
    const canShare = Array.isArray(p.permission?.canPublishSources)
      && (p.permission.canPublishSources.includes(TrackSource.SCREEN_SHARE)
          || p.permission.canPublishSources.includes(TrackSource.SCREEN_SHARE_AUDIO));
    res.json({ ok: true, canShare });
  } catch (e) {
    res.status(e?.status || 500).json({ error: e?.message || 'Failed to query permission' });
  }
});
...
app.get('/api/participant/share-status', async (req, res) => {
  try {
    ...
    try {
      ...
      canShare = sources.includes(TrackSource.SCREEN_SHARE) || sources.includes(TrackSource.SCREEN_SHARE_AUDIO);
    } catch {
